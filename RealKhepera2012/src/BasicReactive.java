...
public class BasicReactive extends RobotController{

  /**
   * Default Constructor
   */
  public BasicReactive(String serialPort, long waitTime) {
...
//	  logDistanceValues();
	   	...
	   	if (getDistanceValue(2) > 500) setMotorSpeeds(2, -5); else
	   		if (getDistanceValue(3) > 500) setMotorSpeeds(-5, 2); else
	   		if (getDistanceValue(1) > 500) setMotorSpeeds(5, -5); else
	   		if (getDistanceValue(4) > 500) setMotorSpeeds(-5, 5); else
	   		if (getDistanceValue(0) > 500) setMotorSpeeds(5, -5); else
	   		if (getDistanceValue(5) > 500) setMotorSpeeds(-5, 5); else
...
//	   	    if (getLightValue(5)    < 200) setMotorSpeeds( 5, 1); else
  ...
  protected void logDistanceValues()  {
	  System.out.println("" + getDistanceValue(0) + ", " + getDistanceValue(1) + ", " + getDistanceValue(2) + ", " 
                                      + getDistanceValue(3) + ", " + getDistanceValue(4) + ", " + getDistanceValue(5)); 
  }
  
  
  public static void main ( String[] args )
  {
  	BasicReactive brc = new BasicReactive("COM10",100);
      try
      {
          brc.robStart();
      }
